...
The Pyrosaurus game appends two bytes to the end of the contest file to record the Arena and Rank of the player at the time of watching an unwatched contest. If the player wins the match - which is known from the contest file but unknown to the player - then the player's rank is increased. If the max rank for the given level is reached, then player is advanced to the next level. See [Level Data Specification](Level%20Specification.md) for more details.

## Contest Header Structure
 
|Field|Size (bytes)|Position (hex)|Description
|--|--|--|--|	
|Contest Status|1|0|0x1=Not watched and first offset Won, 0x2=Not Watched and second offset Won, 0x3=Not watched and Draw. If this value is 0x0, the game never properly ends.||
...
The Contest Data is a series of encoded Animation functions along with the required arguments. It's possible for multiple Animations to be triggered within one Game Tick, so the data structure supports this. I assume the idea at the time is that the same or slightly different simulation code could be run at the server, and the player's program then reads whatever Animation data was generated by the server. Somehow, the animation system appears to be fairly resilient to data issues where it doesn't crash, it just doesn't animate correctly on junk, unexpected, or poorly timed data. If too many frames with Animation on the same dino are encountered in succession, the Animation doesn't work correctly, causing the action data to be "out of sync" with what is happening on the screen. Manually tuned delays are included as a guideline for Dino AI. 

### Game Tick Frame Structure
|Field|Size|Description|
|--|--|--|
|Control / Scalar|1 byte|The value of this must fall within three ranges. 0x0 is the End Of Contest terminator. 0x1 - 0x7F is the number of animation entries within this frame. 0x80 - 0xFF is number of Game Ticks to delay/wait. This is sent as a negative number but the absolute value is what determines the delay.||
|Animation Structures|Number of Animations|This isn't needed if the Control / Scalar is 0x0 or 0x80 - 0xFF (No animation). This is an array of Animation Structures with their arguments included as separate bytes, if required||

### Animations Structure
The Animations Structure within each Game Tick Frame contains the following information:
|Field|Size (bytes)|Description|
|--|--|--|
|Encoded Byte|1|This is a single encoded byte which represents Dino_Index (0-19) and an Animation (0-11). Together these select a Dino in the Contest Dino Array and an Animation to perform on that Dino||
|Arguments|Variable|Depending on the Animation, there may be 0 or up to 3 arguments required||
...
Animation = a number between 0 and 11.

* Encoding Algorithm
  * Encoded_Byte = (Dino_Index * 12) + Animation
* Decoding Algorithm
  * Dino_Index = floor(Encoded_Byte / 12)
  * Animation = Encoded_Byte - (Dino_Index * 12)
...
## Dino Animations
There are 12 animations available to perform on Dinos. This is a list of these animations, what they do and how many arguments (bytes) they require. All arguments are required except where an Animation expects 0 arguments. An omitted or extra argument will bring Contest Data reading out of alignment and cause Undefined Behavior.
|Animation Code|# of Arguments|Description|Delay (frames)|Function Breakpoint|Contest Trace Breakpoint|
|--|--|--|--|--|--|
...
|11|Up to 2|Special Animations| | |9D4B:916||

### Special Animations
The first argument supplied with Animation Code 11 (Special Animations) corresponds to which Animation to perform. All Animation except Animation 7 do not take any additional arguments.
|Animation Code|Argument Code|# of Arguments|Description|Function Breakpoint|Contest Trace Breakpoint|
|--|--|--|--|--|--|
...
|Bit|Use|Description|
|---|---|---|
|8|Passed on the stack as arg_0|An indicator for whether the dino is or has recently been running (IsRunningSpeed). Significant for slowing down from run to walk speed smoothly or slowing down during flocking.||
|7 6 5|Passed via the dx register|Used to force stop a dino. 1 while dino is moving or about to move, 0 to stop dino||
|4 3 2 1|Passed via the bx register|Used to indicate the mode of the movement. 0x0 = stop, 0x1 = creep, 0x4 = run, 0xA = walk||

##### Contest Argument 3
Contest argument 3 is used to directly feed an array movementAnimation which the doMove function uses and also sets to coordinate animation scheduling/triggering. Located at memory location ds:65b8.
...
|stack argument arg_0|Contest Argument 2, OR (0x1 << 7)||
|ds:65b8|Contest Argument 3||

##### How it works

Stop movement speed
When bx is 0, what this means is that the dino stays in place but may rotate in place to face the next movement goal or dino during a fight.
...
|Animation|ax|bx|dx|stack|speed|
|---|---|---|---|---|---|
...
|Animation|ax|bx|dx|stack|speed|
|---|---|---|---|---|---|
Start movement|0|4|1|1|0||
Speed up 1|0|4|1|1|9||
Speed up 2|0|4|1|1|A||
Speed up 3|0|4|1|1|A||
Full speed|0|4|1|1|B||
Slow down 1|0|4|1|1|D||
Slow down 2?|0|4|1|1|F||
Slow down 3|0|4|1|1|5||
Stop movement|0|4|0|1|0||
...
Updates the same byte array as Special Animation 11-9 to 1. Associated with firing? Move neck normal sets to 0
...
#### Set Armor Display (Special Animations 1-6)
Special Animations 1-6 all map to the same operation and set a value in an array. This array controls how a dino's armor displays as an indicator of the dino's heath.
...
Updates the same byte array as Animation 9 to 0. Associated with firing?
